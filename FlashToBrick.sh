...
T="砖寄脚本"
S="开始" E1=":未检测到" E2="请进入fastboot模式并" D="检测到fastboot" ML="命令" A1="设置活动为A槽" F1="刷入" F2="刷入vbmeta" R1="重启到" R3="请手动" R4="fastbootd" C1="清理COW" F3="刷入逻辑" F4="格式化" OK="成功" FAIL="失败" SK="跳过(无此文件)" DEL="删除" CQ="重启" SR="刷入" JJ="检查fastboot设备" BL="bootloader" FQ="分区" JX="继续" SB="失败" S1="设备" YI="已" DD="等待" MS="模式" WC="错误" CS="超时" ZZ="正在" QR="确认" JR="进入" LJ="连接" W="耐心" GE="个" AH="按回车键"
# 镜像文件所在的目录"." 代表与脚本在同一个目录
imgs="images"
L="./flash.log"
#分区刷写列表
#bootloader模式下刷入的文件列表
parts="bluetooth boot。init_boot"
#分区清理列表
...
#逻辑分区
logical="odm product system system_dlkm system_ext vendor vendor_dlkm"
fmt_bytes() {
echo "$1" | awk '
function human(x) {
s="BKMGT";
...
log "${B}${NC}${F4}${FQ}..."
for p in $cows; do
    run "fastboot erase $p" "${F4} ${C}${p}${NC}" "-"
done
log "${B}${NC} ${C1}${FQ}..."
for p in $cows; do
...
log "${B}${NC} ${R1}${BL}${MS}..."
run "fastboot reboot bootloader" "${R1}${BL}${MS}"
