...
T=砖寄脚本 S=开始 E1=":无" D=请 DL=独立 FB=fastboot ML=命令 A1=A槽 F2=vbmeta R1=到 R3=手动 R4=fastbootd C1=COW F3=逻辑 F4=格式化 OK=成功 FAIL=失败 TG=跳过 SC=删除 SF="是否(y/N)" CQ=重启 SR=刷入 JC=检测 BL=bootloader FQ=分区 JX=继续 SB=失败 S1=设备 YI=已 DD=等待 MS=模式 WC=错误 CS=超时 ZZ=正在 QR=确认 JR=进入 LJ=连接 W=耐心 YJ=依据 WZ=未知 GE=个 AH=按回车键 SK="(无此文件)"
# 镜像文件所在的目录"." 代表与脚本在同一个目录
imgs="images"
L="./flash.log"
#刷写列表
#bootloader模式刷写列表
ps="modem dtbo boot vendor_boot init_boot recovery"
#fastbootd刷写列表↓
...
main() {
    ...
    read var
    log "${B}${NC}${JC}${LJ}${S1}"
    if ! wd "bootloader" 9 "${S1}${LJ}${OK}" "${S1}${LJ}${SB}"; then
        log "${Y}${D}${JR}${FB}${MS}${NC}"
        exit 1
    fi
    check_ab
